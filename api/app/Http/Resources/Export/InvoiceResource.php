...
class InvoiceResource extends ExtendedJsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (!$this->manual_input && $this->currency_code != $companyCurrency) {
            $totalNoMods = ceiling(entityPrice(Invoice::class, $this->id, true, $this->currency_rate_customer, false), 2);
            $totalWithMods = ceiling(entityPrice(Invoice::class, $this->id, false, $this->currency_rate_customer, true), 2);
            $totalAffectedByPriceModifiers = ceiling(entity_price_affected_by_modifiers(Invoice::class, $this->id, $this->currency_rate_customer), 2);
            $totalAmountWithoutVat = ceiling(entityPrice(Invoice::class, $this->id, false, $this->currency_rate_customer, false), 2);
        } else {
            $totalNoMods = entityPrice(Invoice::class, $this->id, true, 1, false);
            $totalWithMods = entityPrice(Invoice::class, $this->id, false, 1, true);
            $totalAffectedByPriceModifiers = ceiling(entity_price_affected_by_modifiers(Invoice::class, $this->id, 1), 2);
            $totalAmountWithoutVat = ceiling(entityPrice(Invoice::class, $this->id, false, 1, false), 2);
        }

        $downPaymentAmount = !($this->down_payment === null) ? round(($totalWithMods) * ($this->down_payment / 100), 2) : 0;
        ...
        'VAR_DUE_DATE'           => $this->due_date ? $this->due_date->format('d/m/Y') : null,
        ...
        'VAR_TOTAL_PRICE'        => $this->manual_input ? $this->currencyFormatter->formatCurrency($this->manual_price, $symbol) :
            $this->currencyFormatter->formatCurrency($totalAmountWithoutVat + $vat + $transactionFeeAmount - $downPaymentAmount, $symbol),
