...
class QuoteResource extends ExtendedJsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        if (!$this->manual_input && $this->currency_code != $companyCurrency) {
            $totalNoMods = ceiling(entityPrice(Quote::class, $this->id, true, $this->currency_rate_customer, false), 2);
            $totalWithMods = ceiling(entityPrice(Quote::class, $this->id, false, $this->currency_rate_customer, true), 2);
            $totalAffectedByPriceModifiers = ceiling(entity_price_affected_by_modifiers(Quote::class, $this->id, $this->currency_rate_customer), 2);
            $totalAmountWithoutVat = ceiling(entityPrice(Quote::class, $this->id, false, $this->currency_rate_customer, false), 2);
        } else {
            $totalNoMods = entityPrice(Quote::class, $this->id, true, 1, false);
            $totalWithMods = entityPrice(Quote::class, $this->id, false, 1, true);
            $totalAffectedByPriceModifiers = ceiling(entity_price_affected_by_modifiers(Quote::class, $this->id, 1), 2);
            $totalAmountWithoutVat = ceiling(entityPrice(Quote::class, $this->id, false, 1, false), 2);
        }
        ...
        if (!($this->down_payment === null)) {
            if ($this->down_payment_type == DownPaymentAmountType::percentage()->getIndex()) {
                $downPaymentAmount = ($totalWithMods + $vat) * ($this->down_payment / 100);
            } else {
                $downPaymentAmount = $this->down_payment;
            }
        }
        ...
        return [
          ...
          'VAR_EXPIRY_DATE'        => $this->expiry_date->format('d/m/Y') ?? null,
          'VAR_STATUS'             => $this->status ?? null,
          'VAR_NUMBER'             => $this->number ?? null,
          'VAR_REFERENCE'          => $this->reference ?? null,
          'VAR_CURRENCY'           => !($this->currency_code === null) ?
              CurrencyCode::make($this->currency_code)->getValue() :
              null,
          ...
          'VAR_C_COUNTRY'          => $this->project->contact->customer->billing_address->country ?
              Country::make($this->project->contact->customer->billing_address->country)->getValue() :
              null,
          'VAR_S_FIRST_NAME'       => $this->project->salesPersons[0]->first_name ?? null,
          'VAR_S_LAST_NAME'        => $this->project->salesPersons[0]->last_name ?? null,
          'VAR_S_EMAIL'            => $this->project->salesPersons[0]->email ?? null,
          'VAR_DOWN_PAYMENT'       => !($this->down_payment === null) ? $this->currencyFormatter->formatCurrency($downPaymentAmount, $symbol) : null,
